 ...
 */
import type { CloudDb as Db } from '@jumaah/cloud-db';
import { ROOMS } from '@jumaah/core';
import { type InsightDto, type InsightSessionDto } from '@jumaah/cloud-shared';
import type { CloudContext as AppContext } from '../context.js';

import { viewerCounts, type ViewerCounts } from '@jumaah/api';
import { coreCtx } from '../context.js';
export type Counts = ViewerCounts;
interface _Unused {
  displays: number;
  phones: number;
...
export async function recordViewerPeaks(ctx: AppContext, tenantId: string, sessionId: string, c: Counts): Promise<void> {
  try {
...
export async function viewersChanged(ctx: AppContext, tenantId: string, sessionId: string | null): Promise<number> {
  const c = await viewerCounts(coreCtx(ctx), tenantId);
  if (sessionId) void recordViewerPeaks(ctx, tenantId, sessionId, c);
...
export async function seedSessionStats(ctx: AppContext, tenantId: string, sessionId: string): Promise<void> {
  const c = await viewerCounts(coreCtx(ctx), tenantId);
  if (c.displays || c.phones) await recordViewerPeaks(ctx, tenantId, sessionId, c);
}
