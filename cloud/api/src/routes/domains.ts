...
import { customDomainSchema, type DomainStatusDto } from '@jumaah/cloud-shared';
import { audit } from '@jumaah/api';
import { badRequest, conflict, notFound } from '@jumaah/api';
...
import { tenantFeatures } from '../services/features.js';
...
export async function domainRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.put('/tenant/domain', { preHandler: admin }, async (request) => {
    ...
    await Promise.all([forgetCustomDomain(ctx, t.customDomain), forgetCustomDomain(ctx, domain)]);
    await audit(asCoreDb(db), t.id, actorOf(request), 'tenant.domain.set', 'Tenant', t.id, { customDomain: t.customDomain }, { customDomain: domain });
  ...
  app.post('/tenant/domain/verify', { preHandler: admin }, async (request) => {
    ...
    await audit(asCoreDb(db), t.id, actorOf(request), res.ok ? 'tenant.domain.verified' : 'tenant.domain.verifyFailed', 'Tenant', t.id, null, { domain: t.customDomain, error: res.error });
