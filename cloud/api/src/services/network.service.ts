 ...
 */
import type { CloudDb as Db, Prisma } from '@jumaah/cloud-db';
import type { NetworkSettings, NetworkStatusDto } from '@jumaah/cloud-shared';
import { audit, outbox, type Actor } from '@jumaah/api';
...
export async function saveNetworkTranslation(ctx: AppContext, tenantId: string, paragraphId: string, lang: string, hit: { id: string; text: string; sourceTenantId: string }, userId: string | null) {
  ...
  await outbox(asCoreDb(ctx.db), tenantId, 'Translation', row.id, 'UPSERT', row, row.version);
  return row;
}
...
export async function setNetworkSettings(ctx: AppContext, tenantId: string, next: NetworkSettings, actor: Actor): Promise<NetworkStatusDto> {
  ...
  await audit(asCoreDb(ctx.db), tenantId, actor, 'network.settings', 'Tenant', tenantId, before, { ...merged, removed, added });
