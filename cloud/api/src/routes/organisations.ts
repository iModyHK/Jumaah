...
import { createOrganisationSchema, organisationAdminSchema, organisationTenantSchema, updateOrganisationSchema, type OrganisationDto } from '@jumaah/cloud-shared';
import { audit } from '@jumaah/api';
import { badRequest, conflict, forbidden, notFound } from '@jumaah/api';
import { idParam, parse } from '@jumaah/api';
import { forgetUserAuth } from '@jumaah/api';
...
export async function organisationRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/organisations', { preHandler: superOnly }, async (request, reply) => {
    ...
    await audit(asCoreDb(db), null, actorOf(request), 'organisation.create', 'Organisation', o.id, null, { name: o.name, slug: o.slug });
  ...
  app.patch('/organisations/:id', { preHandler: superOnly }, async (request) => {
    ...
    await audit(asCoreDb(db), null, actorOf(request), 'organisation.update', 'Organisation', id, before, o);
  ...
  app.delete('/organisations/:id', { preHandler: superOnly }, async (request) => {
    ...
    await forgetOrganisationMembers(ctx, id);
    await audit(asCoreDb(db), null, actorOf(request), 'organisation.delete', 'Organisation', id, { name: before.name }, null);
    return { ok: true };
  });
  ...
  app.post('/organisations/:id/tenants', { preHandler: superOnly }, async (request) => {
    ...
    await forgetOrganisationMembers(ctx, id);
    await audit(asCoreDb(db), tenantId, actorOf(request), 'organisation.tenant.add', 'Tenant', tenantId, { organisationId: t.organisationId, plan: t.plan }, { organisationId: id, plan: 'ENTERPRISE' });
    return orgOrThrow(id);
  });
  ...
  app.delete('/organisations/:id/tenants/:tenantId', { preHandler: superOnly }, async (request) => {
    ...
    await Promise.all([forgetOrganisationMembers(ctx, id), forgetUserAuth(coreCtx(ctx), affected.map((u) => u.id))]);
    await audit(asCoreDb(db), tenantId, actorOf(request), 'organisation.tenant.remove', 'Tenant', tenantId, { organisationId: id }, { organisationId: null });
    return orgOrThrow(id);
  });
  ...
  app.post('/organisations/:id/admins', { preHandler: superOnly }, async (request) => {
    ...
    await forgetUserAuth(coreCtx(ctx), [user.id]);
    await audit(asCoreDb(db), user.tenantId, actorOf(request), 'organisation.admin.add', 'User', user.id, null, { organisationId: id });
    return orgOrThrow(id);
  });
  ...
  app.delete('/organisations/:id/admins/:userId', { preHandler: superOnly }, async (request) => {
    ...
    await forgetUserAuth(coreCtx(ctx), [userId]);
    await audit(asCoreDb(db), user.tenantId, actorOf(request), 'organisation.admin.remove', 'User', userId, { organisationId: id }, null);
    return orgOrThrow(id);
  });

  // ---- Organisation admins: their own organisation ----
  app.get('/organisation', { preHandler: app.authenticate }, async (request) => {
    const oid = (request.user!.ext?.organisationId as string | null | undefined);
