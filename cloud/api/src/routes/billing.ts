...
import { PLAN_PRICES_SAR, applySponsorshipSchema, billingSettingsSchema, customInvoiceSchema, markPaidSchema, sponsorSchema, subscribeSchema, type BillingOverviewDto, type PlatformBillingDto, type PublicInvoiceDto, type SponsorResultDto } from '@jumaah/cloud-shared';
import { z } from 'zod';
import { badRequest, forbidden, notFound } from '@jumaah/api';
...
export async function billingRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/billing/cancel', { preHandler: admin }, async (request) => {
    ...
    await setCancelAtPeriodEnd(ctx, request.tenantId, cancel, actorOf(request));
  ...
  app.post('/billing/subscribe', { preHandler: admin }, async (request, reply) => {
    ...
    const inv = await subscribeNow(ctx, request.tenantId, body.plan, body.cycle, actorOf(request));
    reply.code(201);
    return { invoice: invoiceDto(config, inv), seller: await sellerInfo(ctx) };
  });
  ...
  app.post('/billing/invoices/:id/pay', { preHandler: admin }, async (request) => {
    ...
    const withUrl = await ensurePaymentUrl(ctx, inv);
    return { invoice: invoiceDto(config, withUrl), seller: await sellerInfo(ctx) };
  });
  ...
  app.post('/platform/billing/run', { preHandler: superOnly }, async () => runBilling(ctx));
  ...
  app.post('/platform/invoices', { preHandler: superOnly }, async (request, reply) => {
    ...
    const inv = await createCustomInvoice(ctx, body, actorOf(request));
    reply.code(201);
  ...
  app.post('/platform/invoices/:id/mark-paid', { preHandler: superOnly }, async (request) => {
    ...
    const inv = await markPaid(ctx, idParam(request.params), { provider: 'manual', reference: body.reference ?? null }, actorOf(request));
    return invoiceDto(config, inv);
  });

  app.post('/platform/invoices/:id/void', { preHandler: superOnly }, async (request) => invoiceDto(config, await voidInvoice(ctx, idParam(request.params), actorOf(request))));
  ...
  app.post('/platform/sponsorships/:id/apply', { preHandler: superOnly }, async (request) => {
    ...
    const s = await applySponsorship(ctx, idParam(request.params), tenantId, actorOf(request));
  ...
  app.post('/public/sponsor', { config: { rateLimit: { max: config.RATE_LIMIT_AUTH, timeWindow: '1 minute' } } }, async (request, reply): Promise<SponsorResultDto> => {
    ...
    const { invoice } = await createSponsorship(ctx, body);
    const dto = invoiceDto(config, invoice);
    reply.code(201);
    return { invoiceNumber: dto.number, total: dto.total, vat: dto.vat, currency: dto.currency, viewUrl: dto.viewUrl, paymentUrl: dto.paymentUrl, seller: await sellerInfo(ctx) };
  });
  ...
  app.get('/public/invoices/:number', { config: { rateLimit: { max: 60, timeWindow: '1 minute' } } }, async (request): Promise<PublicInvoiceDto> => {
    ...
    return { invoice: invoiceDto(config, inv), seller: await sellerInfo(ctx), qr: await invoiceQr(ctx, inv) };
  });
  ...
  app.get('/public/billing/moyasar/callback', async (request, reply) => {
    ...
    if (inv.status === 'OPEN' && inv.paymentProvider && inv.paymentRef) {
      const state = await verifyPayment(ctx, inv.paymentProvider, inv.paymentRef);
      if (state === 'paid') await markPaid(ctx, inv.id, { provider: inv.paymentProvider, reference: inv.paymentRef }, { id: null, email: 'moyasar:callback', ip: request.ip });
    }
  ...
  app.post('/public/billing/moyasar/webhook', async (request, reply) => {
    ...
    if (inv.status === 'OPEN' && inv.paymentProvider && inv.paymentRef) {
      const state = await verifyPayment(ctx, inv.paymentProvider, inv.paymentRef);
      if (state === 'paid') await markPaid(ctx, inv.id, { provider: inv.paymentProvider, reference: inv.paymentRef }, { id: null, email: 'moyasar:webhook', ip: request.ip });
    }
    return { ok: true };
  });
}
