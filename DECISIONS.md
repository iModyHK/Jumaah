...
## 11. ملاحظات من التحقق الفعلي (E2E)

- زمن تحديث الشاشة بعد أمر الإمام قِيس في Playwright عبر HTTP + Socket + رسم: **~22–30 ms** على نفس الجهاز.
- حالة `ENDED` مؤقتة (8 ثوانٍ) ثم `WAITING`؛ إذا أُعيد تشغيل الـ API قبل انتهاء المؤقت تُعتبر الحالة القديمة (> 10 ثوانٍ) منتهية وتُعاد إلى `WAITING` عند أول قراءة.
- متجر الشاشة يُعاد تسليحه في `start()` لأن React StrictMode يشغّل mount → unmount → mount على نفس المتجر المحفوظ بـ `useMemo`.
