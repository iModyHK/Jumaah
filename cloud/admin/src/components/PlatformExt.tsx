...
import { Card, Field, TextInput, api, fmtDate, useToast } from '@jumaah/admin';

/** The image tag the cloud announces to mosque servers (their "update available" banner). */
export function LatestImageTagCard() {
  const { t } = useTranslation();
  const toast = useToast();
  const qc = useQueryClient();
  const settings = useQuery({ queryKey: ['platform', 'settings'], queryFn: () => api.get<Record<string, unknown>>('/platform/settings') });
  const stats = useQuery({ queryKey: ['platform', 'stats'], queryFn: () => api.get<{ latestImageTag?: string }>('/platform/stats') });
  const [tag, setTag] = useState('');
  useEffect(() => {
    const v = settings.data?.['edge.latestImageTag'] as { tag?: string } | undefined;
    if (v?.tag !== undefined) setTag(v.tag);
    else if (stats.data?.latestImageTag) setTag(stats.data.latestImageTag);
...
/** Every mosque's plan, state and platform-AI usage this month, the view used for manual billing. */
