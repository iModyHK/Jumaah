/**
 * Integration tests of Jumaah Cloud against a real PostgreSQL + Redis, on top of the Community API.
 * Requires the cloud seed to have run (demo tenant on PRO with a sync key).
 */
import { afterAll, beforeAll, describe, expect, it } from 'vitest';
import type { FastifyInstance } from 'fastify';
import { buildApp, createRedis, loadConfig } from '@jumaah/api';
import { asCoreDb, createPrisma } from '@jumaah/cloud-db';
import { cloudExtension, loadCloudConfig } from './index.js';
...
beforeAll(async () => {
  ...
  const config = loadCloudConfig(loadConfig(env), env);
  const cloudDb = createPrisma(DATABASE_URL);
  const db = asCoreDb(cloudDb);
  app = await buildApp({ config, db, redis: createRedis(REDIS_URL, 'test'), pub: createRedis(REDIS_URL, 'tpub'), sub: createRedis(REDIS_URL, 'tsub') }, { extensions: [cloudExtension(config)] });
  await app.ready();
...
beforeAll(async () => {
  // A khutbah with sections and paragraphs for the tests that need one (created through the API like the core tests do).
  const created = await app.inject({
    method: 'POST',
    url: '/api/khutbahs',
    headers: auth(adminToken),
    payload: {
      title: 'خطبة الاختبار السحابي',
      gregorianDate: '2026-10-02',
      targetLanguages: ['en', 'ur'],
      sections: [
        { type: 'FIRST', text: 'الحمد لله رب العالمين.\n\nأما بعد، فاتقوا الله عباد الله.' },
  ...
  const k = created.json() as { id: string; sections: Array<{ id: string; type: string; paragraphs: Array<{ id: string }> }> };
  khutbahId = k.id;
  sectionFirstId = k.sections.find((x) => x.type === 'FIRST')!.id;
  paragraphIds = k.sections.flatMap((x) => x.paragraphs.map((q) => q.id));
});
...
describe('hosted plans: platform AI gate and metering', () => {
  ...
  it('a plan without AI cannot start platform translation, but the estimate still answers', async () => {
    ...
    expect(est.json().ext.ai).toMatchObject({ applies: true, allowed: false, reason: 'NOT_INCLUDED' });
  ...
  it('an ended subscription keeps AI for the grace period, then switches it off', async () => {
    ...
    expect(own.json().ext.ai.allowed).toBe(false);
...
describe('hosted plans: trial defaults and platform overview', () => {
  ...
  it('a new mosque starts a 30-day Standard trial', async () => {
    ...
    createdId = t.id;
    expect(t.ext.plan).toBe('STANDARD');
    expect(t.ext.subscriptionStatus).toBe('TRIAL');
    const days = (new Date(t.ext.subscriptionEndsAt).getTime() - Date.now()) / 86_400_000;
...
describe('branding: gated by plan, filtered on public info', () => {
  ...
  it('rejects branding above the plan and accepts what the plan includes', async () => {
    ...
    expect(denied.json().error.code).toBe('FEATURE_NOT_AVAILABLE');
...
describe('paid-edition extras: public archive, handouts and insight by plan', () => {
  ...
  it('Basic cannot publish the archive, print handouts or read insight; the public archive stays hidden', async () => {
    ...
    expect(on.json().error.code).toBe('FEATURE_NOT_AVAILABLE');
  ...
  it('Standard publishes delivered khutbahs only, with approved translations only', async () => {
    ...
    expect((await app.inject({ method: 'GET', url: '/api/public/tenant/demo' })).json().tenant.ext.archiveEnabled).toBe(true);
...
describe('custom domains (Pro) and organisation accounts (hosted edition)', () => {
  ...
  it('an organisation admin switches between member mosques, shares the AI pool, and not beyond', async () => {
    ...
    expect(l.user.ext).toMatchObject({ organisationId: orgId });
    ...
    expect((await login('admin@demo.mosque', 'Demo12345!')).user.ext?.organisationId).toBeNull();
