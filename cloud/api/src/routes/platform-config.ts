...
import { cloudCtx, coreCtx } from '../context.js';
import { asCoreDb } from '@jumaah/cloud-db';
import { PLATFORM_CONFIG_GROUPS, platformGroupSchemas, testEmailSchema, type PlatformConfigGroup } from '@jumaah/cloud-shared';
import { z } from 'zod';
import { audit } from '@jumaah/api';
import { badRequest } from '@jumaah/api';
import { parse } from '@jumaah/api';
import { sendEmail } from '@jumaah/api';
...
export async function platformConfigRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/platform/config', { preHandler: superOnly }, async () => platformConfigDto(ctx));
  ...
  app.put('/platform/config/:group', { preHandler: superOnly }, async (request) => {
    ...
    await savePlatformGroup(ctx, group, body as Record<string, unknown>);
    ...
    await audit(asCoreDb(db), null, actorOf(request), 'platform.config', 'PlatformSetting', group, null, safe);
    return platformConfigDto(ctx);
  });
  ...
  app.post('/platform/config/email/test', { preHandler: superOnly }, async (request) => {
    ...
    const status = await sendEmail(coreCtx(ctx), { to, locale, template: 'test', data: { sentBy: request.user!.email } });
    const last = await db.emailLog.findFirst({ where: { to: to.toLowerCase(), template: 'test' }, orderBy: { createdAt: 'desc' } });
    return { status, error: last?.error ?? null, configured: !!(await platformConfig(ctx)).email.host };
  });
