...
export async function userRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/users/invite', { preHandler: admin }, async (request, reply) => {
    ...
    const tenant = await db.tenant.findUniqueOrThrow({ where: { id: request.tenantId }, select: { id: true, slug: true } });
    return reply.code(201).send({ id: inv.id, email, role: body.role, expiresAt: inv.expiresAt.toISOString(), inviteUrl: `${await tenantBaseUrl(app.ctx, tenant)}/admin/invite/${token}` });
  });
