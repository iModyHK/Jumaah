 ...
 */
import type { CloudDb as Db, Invoice, Prisma, Sponsorship } from '@jumaah/cloud-db';
import { randomToken } from '@jumaah/cloud-db';
import { CURRENCY, INVOICE_DUE_DAYS, OVERDUE_GRACE_DAYS, RENEWAL_LEAD_DAYS, SPONSOR_PRICE_SAR, addCycle, addYears, computeTotals, priceFor, toHalalas, zatcaQr, type BillingCycle, type InvoiceDto, type InvoiceKind, type InvoiceLine, type SellerInfoDto, type SponsorshipDto, type SubscriptionPlan } from '@jumaah/cloud-shared';
import type { CloudConfig as Config } from '../config.js';
import { audit, type Actor } from '@jumaah/api';
import type { CloudContext as AppContext } from '../context.js';
import { conflict, notFound } from '@jumaah/api';
import { sendEmailLater } from '@jumaah/api';
import { billingRecipients, organisationRecipients } from './email-recipients.js';
...
export async function applyCancellations(ctx: AppContext, now: Date = new Date()): Promise<number> {
  ...
  for (const t of due) {
    ...
    await audit(asCoreDb(ctx.db), t.id, { id: null, email: 'billing:scheduler' }, 'billing.cancelled', 'Tenant', t.id, { plan: t.plan }, { plan: 'FREE' });
  }
  return due.length;
}
...
export async function setCancelAtPeriodEnd(ctx: AppContext, tenantId: string, cancel: boolean, actor: Actor): Promise<void> {
  ...
  await audit(asCoreDb(ctx.db), tenantId, actor, cancel ? 'billing.cancelRequested' : 'billing.cancelWithdrawn', 'Tenant', tenantId, null, { endsAt: t.subscriptionEndsAt });
}
...
export async function createCustomInvoice(
  ...
  await audit(asCoreDb(ctx.db), input.tenantId ?? null, actor, 'invoice.custom', 'Invoice', inv.id, null, { number: inv.number, total: inv.total, organisationId: input.organisationId ?? null });
  return inv;
}
...
export async function runTrials(ctx: AppContext, now: Date = new Date()): Promise<{ noticed: number; ended: number }> {
  ...
  for (const t of ending) {
    ...
    for (const r of await billingRecipients(ctx, t.id)) {
      sendEmailLater(coreCtx(ctx), { to: r.to, locale: r.locale, template: 'trialEnding', tenantId: t.id, data: { mosqueName: t.name, endsAt: t.subscriptionEndsAt!.toISOString(), plan: PLAN_LABEL[t.plan] ?? t.plan, adminUrl: tenantAdminUrl(ctx, t), daysLeft } });
    }
  }
  ...
  for (const t of over) {
    ...
    await audit(asCoreDb(ctx.db), t.id, { id: null, email: 'billing:scheduler' }, 'billing.trialEnded', 'Tenant', t.id, { plan: t.plan }, { plan: 'FREE' });
    for (const r of await billingRecipients(ctx, t.id)) sendEmailLater(coreCtx(ctx), { to: r.to, locale: r.locale, template: 'trialEnded', tenantId: t.id, data: { mosqueName: t.name, adminUrl: tenantAdminUrl(ctx, t) } });
    ended += 1;
  }
...
export async function markOverdue(ctx: AppContext, now: Date = new Date()): Promise<number> {
  ...
  for (const inv of overdue) {
    ...
    if (res.count > 0) {
      ...
      for (const r of recipients) sendEmailLater(coreCtx(ctx), { to: r.to, locale: r.locale, template: 'pastDue', tenantId: inv.tenantId, data: { customerName: (inv.billTo as { name?: string })?.name ?? '', number: inv.number, total: inv.total, viewUrl: invoiceViewUrl(ctx.config, inv), graceDays: OVERDUE_GRACE_DAYS } });
    }
  }
  return n;
}
...
export async function markPaid(ctx: AppContext, invoiceId: string, payment: { provider: string; reference: string | null }, actor: Actor): Promise<Invoice> {
  ...
  await audit(asCoreDb(ctx.db), inv.tenantId, actor, 'invoice.paid', 'Invoice', inv.id, { status: inv.status }, { status: 'PAID', provider: payment.provider, reference: payment.reference });
  void (async () => {
    ...
    for (const r of recipients) sendEmailLater(coreCtx(ctx), { to: r.to, locale: r.locale, template: 'paymentReceived', tenantId: inv.tenantId, data: { customerName: (inv.billTo as { name?: string })?.name ?? '', number: inv.number, total: inv.total, viewUrl: invoiceViewUrl(ctx.config, inv), paidUntil } });
...
export async function voidInvoice(ctx: AppContext, invoiceId: string, actor: Actor): Promise<Invoice> {
  ...
  await audit(asCoreDb(ctx.db), inv.tenantId, actor, 'invoice.void', 'Invoice', inv.id, { status: inv.status }, { status: 'VOID' });
  return v;
}
...
export async function createSponsorship(ctx: AppContext, input: { sponsorName: string; sponsorEmail: string; sponsorPhone?: string | null; mosqueName?: string | null; message?: string | null; mosques: number; lang: 'ar' | 'en' }): Promise<{ sponsorship: Sponsorship; invoice: Invoice }> {
  ...
  sendEmailLater(coreCtx(ctx), { to: sponsorship.sponsorEmail, locale: input.lang, template: 'sponsorship', data: { sponsorName: sponsorship.sponsorName, mosques: sponsorship.mosques, number: invoice.number, total: invoice.total, viewUrl, paymentUrl: invoice.paymentUrl, iban: seller.iban, bank: seller.bank, sellerName: seller.name } });
  const { email } = await platformConfig(ctx);
  if (email.notifyEmail) sendEmailLater(coreCtx(ctx), { to: email.notifyEmail, locale: 'en', template: 'sponsorshipNotice', data: { sponsorName: sponsorship.sponsorName, sponsorEmail: sponsorship.sponsorEmail, mosques: sponsorship.mosques, mosqueName: sponsorship.mosqueName, message: sponsorship.message, number: invoice.number, total: invoice.total } });
...
export async function applySponsorship(ctx: AppContext, sponsorshipId: string, tenantId: string, actor: Actor): Promise<Sponsorship> {
  ...
  await audit(asCoreDb(ctx.db), t.id, actor, 'sponsorship.apply', 'Sponsorship', s.id, null, { tenantId: t.id, seat: next.length, of: s.mosques });
  return updated;
}
...
export async function subscribeNow(ctx: AppContext, tenantId: string, plan: SubscriptionPlan, cycle: BillingCycle, actor: Actor): Promise<Invoice> {
  ...
  await audit(asCoreDb(ctx.db), tenantId, actor, 'billing.subscribe', 'Invoice', inv.id, { plan: t.plan, replaced: open.length }, { plan, cycle, number: inv.number });
  return inv;
}
