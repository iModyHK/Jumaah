...
import { renderTemplate, type TemplateName } from './email-templates.js';

describe('email templates', () => {
  it('render every template in both languages with subject, text and html', () => {
    const samples: Array<[TemplateName, unknown]> = [
      ...
      ['passwordReset', { name: 'Ahmed', resetUrl: 'https://alnoor.jumaah.net/admin/reset/abc', mosqueName: 'Al Noor', expiresMinutes: 60 }],
      ...
      ['test', { sentBy: 'admin@jumaah.app' }],
    ];
    for (const [name, data] of samples) {
      for (const locale of ['ar', 'en'] as const) {
        const r = renderTemplate(name, locale, data as never);
  ...
  it('escapes user-supplied text and formats money and dates', () => {
    const r = renderTemplate('sponsorshipNotice', 'en', { sponsorName: 'Al Khair', sponsorEmail: 'g@x.org', mosques: 2, mosqueName: null, message: '<script>alert(1)</script>', number: 'JC-1', total: 398000 });
    expect(r.html).not.toContain('<script>');
    expect(r.html).toContain('&lt;script&gt;');
    expect(r.text).toContain('3,980.00 SAR');
    const ar = renderTemplate('invoiceIssued', 'ar', { customerName: 'x', number: 'JC-1', total: 19900, dueAt: '2026-10-01T00:00:00.000Z', viewUrl: 'https://x', paymentUrl: 'https://pay', iban: null, bank: null, sellerName: 'Jumaah', description: 'x' });
